import { createContext, useEffect } from "react";
import {
  ICallbackFunction,
  IContext,
  IPaginateData,
  ISupportThread,
  ISupportChat,
} from "interfaces";
import { useContextData } from "hooks";
import { ApiUrl } from "constant";
import { useAppContext } from "context";
import { string } from "yup";

export const SupportContext = createContext<
  IContext<IPaginateData<ISupportThread>, ISupportChat>
>({
  isLoading: false,
  lists: undefined,
  details: undefined,
  isDetailsLoading: false,
  currentPage: 1,
  async editHandler(payload, cb) {},
  async getDetailsHandler(id) {},
  async getListHandler(query) {},
  async deleteHandler(id, cb) {},
  async toggleVisibility(id, visibilityStatus, cb) {},
});

export default function ContextProvider({ children }) {
  const {
    isLoading,
    isDetailsLoading,
    getDetailsHandler,
    details,
    getListHandler,
    lists,
    setDetails,
    saveHandler,
    deleteHandler,
    editHandler,
    currentPage,
  } = useContextData();
  const { userDetails } = useAppContext();

  const Handlers = {
    async getListHandler(query: any) {
      return getListHandler(ApiUrl.support.get_threadList, query);
    },

    async getDetailsHandler(payload: any) {
      return getDetailsHandler(ApiUrl.support.get_chat, payload);
    },

    async editHandler(payload: any, cb: ICallbackFunction) {
      return saveHandler(
        ApiUrl.support.post_sendMessage,
        payload,
        {
          onSuccess: async (res: any) => {
            const tempDetails: ISupportChat = { ...details };
            res.sender = userDetails;
            tempDetails.messages.docs.push(res);
            setDetails(tempDetails);
            typeof cb?.onSuccess === "function" && (await cb.onSuccess(res));
          },
        },
        {
          hideMessage: true,
        },
      );
    },

    async deleteHandler(id: string, cb: ICallbackFunction) {
      return deleteHandler(ApiUrl.support.delete_message(id), {
        onSuccess: async (res: any) => {
          const tempDetails: ISupportChat = { ...details };
          tempDetails.messages.docs = tempDetails.messages.docs
            .filter((e) => e?._id !== id)
            .reverse();
          setDetails(tempDetails);
          typeof cb?.onSuccess === "function" && (await cb.onSuccess(res));
        },
      });
    },
    async toggleVisibility(id: string, status: string, cb: ICallbackFunction) {
      return editHandler(
        ApiUrl.support.put_changeStatus,
        {
          threadID: id,
          status: status,
        },
        cb,
      );
    },
  };
  return (
    <SupportContext.Provider
      value={{
        isDetailsLoading,
        isLoading,
        lists,
        details,
        currentPage,
        ...Handlers,
      }}
    >
      {children}
    </SupportContext.Provider>
  );
}
